...
目前需要做的工作比较简单，主要是保存文章、评论、用户等数据，并提供相应的 API 接口。
...
当然不能每次发文章都重启服务器，文章仓库放在 Github 的好处是可以设置 [Webhook](https://docs.github.com/en/developers/webhooks-and-events/about-webhooks)，这样每次写完文章 push 到仓库时就会触发钩子通知服务端，此时服务端就可以执行预先设定好的脚本，将文章更新 pull 下来，然后更新数据库和资源文件。

### 前端站点

前端站点也就是[本站](https://me.harley-xk.studio/)，负责数据的获取和展示。前端基于 Nuxt.js 开发，细节没啥多说的，网上关于 Vue 和 Nuxt 的文章已经很多了，而且他们官方文档也有很详尽的中文版本。

#### Markdown

博客网站最主要的就是展示 Markdown 格式文章内容了，这里本站选择了主流的 [markdown-it](https://github.com/markdown-it/markdown-it) 插件，配合 [highlight.js](https://highlightjs.org/) 实现代码块的语法高亮，另外使用[markdown-it-anchor](https://github.com/valeriangalliat/markdown-it-anchor) 在处理 markdown 时为标题添加锚点，实现了文章的目录同步和导航定位。
...
评论系统也是导致我弃用 Hexo 的痛点之一，不过这也不是 Hexo 本身的问题。由于大多数博客网站都是静态托管的，所以目前主流的博客框架基本都是不自带评论系统的，一般都只提供了接入各种第三方评论系统的接口。

不过我简单调研下来发现很多第三方评论系统的 UI 真的是一言难尽...而且大多数还不支持 Markdown 格式的评论。后来选了个 UI 还算看得过去的 [Disquz](https://disqus.com/)，也是这个市场的巨头之一了。不过等博客架设起来之后才发现一个悲催的问题：_**它被墙了**_ 🤐 。鉴于这里涉及到了敏感领域，所以被墙也可以理解吧，不过 Disquz 似乎也不支持 Markdown 评论。

后来还发现了 [gitment](https://github.com/imsun/gitment) 和 [gitalk](https://github.com/gitalk/gitalk) 这种利用 Github issues 功能来搭建评论系统的，不得不佩服广大开发者们清奇的思路。虽然这种方式可以很好的支持 Markdown 格式的评论，但这种技术方案的讨论帖下面都充满了是否属于滥用 Github 功能的争论。虽然 Github 官方并没有站出来明确反对这种行为，但是个人认为对于 Github 这种目前开发者们心中圣地搬的存在，还是矜持一些的好。另外，依托 Github 做评论系统还要面对本文开篇时提到的另一个问题：访问速度。

所以综合看下来，最终决定连评论系统也一起重写了，由于目前用户系统还未完善，暂时只可以对文章发表匿名评论，不过请勿发表敏感言论，网站的所有访问都会记录 IP 地址 👽
