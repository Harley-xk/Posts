...
苹果在每一年的 WWDC 都特别强调用户隐私，到了今年，苹果终于对相册动手了。现有的方案虽然看起来很完美，但是存在一个重大的隐患：用户往往只需要从相册选一张照片上传，可此时 App 却获取了整个相册**所有照片**的数据！！
...
获取视频稍微复杂一点，框架开发者在官方论坛中明确指出需要使用 `loadFileRepresentation` 方法来加载大文件，例如视频：
